...
public class valueDependentTimer : MonoBehaviour {
    ...
    public bool deltaTimeCycleFromValue = false;
    public valueDefinitions.values timeCycleValue;
	...
	IEnumerator cyclicTestValue(){
		...
		while (true) {
            ...
            if (deltaTimeCycleFromValue == true)
            {
                ValueScript vs = valueManager.instance.getFirstFittingValue(timeCycleValue);
                if (vs != null)
                {
                    deltaTimeCycle = vs.value;
                }
            }
