...
[CustomPropertyDrawer(typeof(ValueScript.valueToText))]
public class valueToTextDrawer : PropertyDrawer
{
    // Draw the property inside the given rect
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Draw fields - passs GUIContent.none to each so they are drawn without labels
        EditorGUI.PropertyField(textRect, property.FindPropertyRelative("minValue"), GUIContent.none);
        EditorGUI.PropertyField(iconRect, property.FindPropertyRelative("text"), GUIContent.none);
