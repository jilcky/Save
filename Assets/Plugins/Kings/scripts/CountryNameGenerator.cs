...
public class CountryNameGenerator : TranslatableContent {
    ...
    [Tooltip("Combined text field to display country and name.")]
    public Text countryAndNameText;
    [Tooltip("Formatting of country and name. Standard is '{0} {1}'")]
    public string countryAndNameTextFormat = "{0} {1}";
    ...
    {
        return getCountryTranslatedStringFromValue(); 
    }
    ...
    public string GetCountryNameString()
    {
        ...
        string countryAndNameText = "";
        countryAndNameText = string.Format(countryAndNameTextFormat, name, country);

        return countryAndNameText;
    }
	...
	public nameGenderLink getNameAndGenderFromValue(){
        if (vs_name != null)
        {
            ...
            Debug.LogWarning("vs_name is 'null'");
        }
		return null;
	}
	/*
	...
	public override List<string> getTranslatableTerms ()
	{
		...
		foreach (subStringList ssl in Countries) {
            ...
            //add surenames
            foreach (string surname in ssl.surname) {
                terms.Add(surname);
            }
            //add given names
            foreach (nameGenderLink ngl in ssl.nameComb) {
                terms.Add(ngl.name);
            }
